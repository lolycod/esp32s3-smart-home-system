...
class AIService {
    ...
    getCurrentDeviceState(sessionId) {
        if (!this.deviceStates.has(sessionId)) {
            ...
            // 确保现有会话也包含空调（向后兼容）
            const currentState = this.deviceStates.get(sessionId);
            if (!currentState.空调) {
                currentState.空调 = {
                    "开关": "关",
                    "温度": 25,
    ...
    updateDeviceState(sessionId, newState) {
        ...
        if (newState.空调) {
            Object.assign(currentState.空调, newState.空调);
        }
...
4. 空调：开关、温度、模式、风速、排风方向（开关："开|关"，温度：16-30度，模式："制冷|制热"，风速："自动|低速|中速|高速"，上下排风："自动|固定|摆动"，左右排风："自动|固定|摆动"）
...
**控制设备时的JSON格式：**
{
  ...
  "窗帘": {"开关": "开|关", "理由": "简要理由"},
  "空调": {"开关": "开|关", "温度": 16-30, "模式": "制冷|制热", "风速": "自动|低速|中速|高速", "上下排风": "自动|固定|摆动", "左右排风": "自动|固定|摆动", "理由": "简要理由"},
  "综合说明": "环境调整说明(20字以内)"
}
...
**错误示例（禁止这样回复）：**
❌ 您好，老板！理解您现在的感受。我建议... {"空调": {...}}
❌ 好的，我来为您调整。 {"风扇": {...}}

**正确示例（必须这样回复）：**
✅ {"空调": {"开关": "开", "温度": 25, "理由": "为您开启空调"}}
✅ {"风扇": {"开关": "开", "风速": 2, "理由": "开启风扇降温"}}
